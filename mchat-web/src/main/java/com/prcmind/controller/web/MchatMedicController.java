...
public class MchatMedicController {
	...
	@ResponseBody
	public CodeMsgBean<Object> getMchatQuestionnaireResponse(String scoreNo,String medicNo,HttpServletRequest request)
			throws IOException {
		LOGGER.info("getMchatQuestionnaireResponse接口请求参数scoreNo="+scoreNo);
		if (StringUtils.isEmpty(scoreNo)) {
			return new CodeMsgBean<Object>(10003, "参数异常");
		}
		String medic_no = getMedicNo(request);
		if (StringUtils.isEmpty(medic_no)) {
			// medicNo = "937c2b21d3db406693c59a816614e26d";
			return new CodeMsgBean<Object>(10002,"登录失效，请重新登录");
		}
		if(StringUtils.isEmpty(medicNo)){
	...
	@ResponseBody
	public CodeMsgBean<Object> downloadReport(String scoreNo, HttpServletRequest request,String medicNo, HttpServletResponse response)
			throws IOException {
		LOGGER.info("downloadReport接口请求参数scoreNo="+scoreNo+",medicNo="+medicNo);
		if (StringUtils.isEmpty(scoreNo)) {
			return new CodeMsgBean<Object>(10003, "参数异常");
		}
		String medic_no =getMedicNo(request);
		...
		try {
			...
			if (result != null) {
				...
				String path = "";
				String out_path=request.getSession().getServletContext().getRealPath("user");
				if(result.getScore() != null){
					
					if (result.getScore() <= 2) {
						path = request.getSession().getServletContext().getRealPath("template/A.pdf") ;
//						path ="E:\\downloads\\template\\A.pdf";
...
//						path ="E:\\downloads\\template\\C.pdf";
	...
	private Map<String, String> initMap(MchatScoreRevisedFollow mchatScoreRevisedFollow, MchatScore result) {
		...
		content.put("sex", result.getSex() == 1 ? "男" : "女");
...
//		mchatScore.setTestToday(mapTestDate.get("testToday"));
	...
	@ResponseBody
	public CodeMsgBean<Object> getMchatQuestionnaireResponseRevisedFollow(String scoreNo,String medicNo, HttpServletRequest request)
			throws IOException {
		LOGGER.info("getMchatQuestionnaireResponseRevisedFollow接口请求参数scoreNo="+scoreNo+",medicNo="+medicNo);
		if (StringUtils.isEmpty(scoreNo)) {
			return new CodeMsgBean<Object>(10003, "参数异常");
		}
		String medic_no = getMedicNo(request);
